...
export const validateRequired = (value: string, fieldName: string = 'Поле'): string | null => (value.trim().length > 0 ? null : `${fieldName} обязательно`);
...
export const validateMessage = (message: string): string | null => (message.trim().length > 0 ? null : 'Сообщение не может быть пустым');
